...
function Stat({
  ...
  return (
    <div className="bg-card px-4 py-3">
      ...
      <div
        className={`font-display mt-1 text-2xl font-semibold leading-none tabular-nums ${valueClass}`}
      >
        {value}
      </div>
    </div>
      ...
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border/70 px-6 pb-4 pt-5">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="text-xs font-semibold text-[#b78625]">{eyebrow}</div>
    ...
    <div className="overflow-hidden rounded-xl border border-border/70 bg-card ring-1 ring-foreground/5">
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border/50 bg-[#fdf6e8]/40 px-5 py-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {stub && (
            <span
              className="inline-flex items-center gap-1 rounded-full bg-[#f8eed7] px-2 py-0.5 text-xs text-[#7a5818] ring-1 ring-inset ring-[#b78625]/30"
      ...
      <div className="px-5 py-4">
        ...
        {integration.lastError && (
          <div className="mt-4 rounded-md border border-[#b94f58]/30 bg-[#fdecee] px-3 py-2 text-xs text-[#8a3942]">
            ...
            <p className="break-words font-mono">{integration.lastError}</p>
          </div>
        )}
    ...
    <div className="mt-4 rounded-md border border-border/60 bg-[#fdf6e8]/40 px-4 py-3">
      ...
      {showInfo && (
        <div className="mt-3 rounded-md border border-[#b78625]/30 bg-[#fff8e8] p-3">
          <div className="text-[10px] font-medium uppercase tracking-wide text-[#7a5818]">
...
function KindBadge({ kind }: { kind: Kind }) {
  ...
  const tone =
    kind === "softpro_360"
      ? { bg: "bg-[#e8f0f8]", text: "text-[#2c4a6b]", ring: "ring-[#3f668f]/30" }
      : kind === "softpro_standard"
        ? { bg: "bg-[#f2e7f1]", text: "text-[#40233f]", ring: "ring-[#593157]/30" }
        : kind === "qualia"
          ? { bg: "bg-[#e6f3ed]", text: "text-[#2f5d4b]", ring: "ring-[#3f7c64]/30" }
          : kind === "resware"
            ? { bg: "bg-[#fde9dc]", text: "text-[#7a3d18]", ring: "ring-[#c9652e]/30" }
            : kind === "encompass"
              ? { bg: "bg-[#f8eed7]", text: "text-[#7a5818]", ring: "ring-[#b78625]/30" }
...
function StatusPill({
  ...
  const tone =
    status === "active"
      ? { ring: "ring-[#3f7c64]/40", text: "text-[#2f5d4b]", bg: "bg-[#e6f3ed]", dot: "bg-[#3f7c64]", label: "Active" }
      : status === "error"
        ? { ring: "ring-[#b94f58]/45", text: "text-[#8a3942]", bg: "bg-[#fdecee]", dot: "bg-[#b94f58]", label: "Error" }
        : { ring: "ring-border", text: "text-muted-foreground", bg: "bg-muted", dot: "bg-muted-foreground", label: "Disabled" }
  return (
    <span
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm ring-1 ring-foreground/5">
      <div className="grid grid-cols-1 gap-8 p-8 md:grid-cols-[1.2fr_1fr] md:p-10">
        <div>
          <div className="text-xs font-semibold text-[#b78625]">Get started</div>
          ...
          <p className="mt-3 max-w-md text-sm leading-relaxed text-muted-foreground">
            An integration is a connection to a system that produces title
            files — SoftPro, Qualia, Encompass, and so on. Each connection has
...
function NewIntegrationForm({
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const config =
        kind === "softpro_360" ? { baseUrl, accountId } : undefined
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="md:col-span-2">
            <div className="rounded-md border border-[#b78625]/35 bg-[#fdf6e8] px-3 py-2 text-sm text-[#7a5818]">
              ...
              <p className="text-xs leading-snug">
                You can reserve the row, but syncs will fail until the
                transport is wired in. Pick another system to continue.
              </p>
            </div>
          </div>
        )}
        ...
        {error && (
          <p className="md:col-span-2 rounded-md border border-[#b94f58]/30 bg-[#fdecee] px-3 py-2 text-sm text-[#8a3942]">
            {error}
          </p>
        )}
