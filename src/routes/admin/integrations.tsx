...
function countByKind(
  list: ReadonlyArray<IntegrationRow>,
): Record<Kind, number> {
...
function KpiStrip({
  ...
  const tiles: ReadonlyArray<{
    ...
    },
    {
      label: "Errors",
      value: stats.error,
      caption: stats.error > 0 ? "attention needed" : "every adapter healthy",
    ...
    <div className="mt-3 rounded-md border border-[#3f7c64]/40 bg-[#e9f3ed] p-3">
      ...
      {!baseUrl && (
        <p className="mt-2 text-[11px] text-[#8a3942]">
          ...
          this build, so the install URL is blank. Fill it in manually with your
          deployment's <code>.convex.site</code> URL.
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="mt-4 rounded-md border border-border/60 bg-[#e8f0f8]/40 px-4 py-3">
      ...
      <p className="mt-3 text-[11px] leading-snug text-muted-foreground">
        Until the Postmark inbound provider is provisioned for your deployment,
        the address above is illustrative. Test the pipeline end-to-end with{" "}
        <code className="rounded bg-card px-1 py-0.5 font-mono text-[10px]">
          bun run admin simulate-email{" "}
          {(integration as { tenantSlug?: string }).tenantSlug ??
            "<tenant-slug>"}
        </code>
        .
      </p>
    ...
    <div className="mt-3 rounded-md border border-[#3f668f]/30 bg-card/80 p-3">
      ...
      {tab === "gmail" && (
        <ol className="flex flex-col gap-1.5 text-[12px] leading-snug text-foreground/85">
          <Step n={1}>
            ...
            <strong>Add a forwarding address</strong> and paste{" "}
          ...
          <Step n={3}>
            Check <strong>Forward it to: {address}</strong> and Apply. New mail
            matching the filter starts arriving in your inbox tab here.
          </Step>
        </ol>
      )}
      ...
      {tab === "outlook" && (
        <ol className="flex flex-col gap-1.5 text-[12px] leading-snug text-foreground/85">
          ...
          <Step n={3}>
            Save. Microsoft 365 begins forwarding immediately — no confirmation
            email required.
          </Step>
        </ol>
      )}
      ...
      {tab === "manual" && (
        <ol className="flex flex-col gap-1.5 text-[12px] leading-snug text-foreground/85">
          ...
          <Step n={3}>
            Send a test message. The first one arrives within 30s and shows up
            in your Mail tab here.
          </Step>
        </ol>
      )}
    </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-x-2">
            ...
            {existing > 0 && (
              <span
                className={`font-numerals text-[10px] tabular-nums ${tone.text}`}
              >
                connected · {existing}
              </span>
            )}
