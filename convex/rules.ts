...
/**
 * Pure resolver per spec §6: returns the rule version effective at `asOf` for
 * a given county + docType, or null if none is configured. Files in flight
 * always resolve against the rule effective at openedAt.
 */
...
export const publishRule = mutation({
  ...
  handler: async (
    ...
    if (supersedes) {
      ...
      if (
        prior.countyId !== countyId ||
        prior.docType !== docType
      ) {
