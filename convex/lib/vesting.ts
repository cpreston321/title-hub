 ...
 *
 * Examples: "John Smith and Jane Smith" → 2 parties, vestingForm: undefined
 * "John Smith and Jane Smith, JTROS" → 2 parties, vestingForm: "JTROS" "John
 * Smith and Jane Smith as TBE" → 2 parties, vestingForm: "TBE" "Acme LLC" → 1
 * party, vestingForm: undefined
 */
...
const VESTING_FORM_PATTERNS: ReadonlyArray<{ re: RegExp; form: VestingForm }> =
  [
    ...
    {
      re: /\bJOINT\s+TENANT(?:S)?(?:\s+WITH\s+RIGHT\s+OF\s+SURVIVORSHIP)?\b/i,
      form: 'JTROS',
    },
 ...
 * and missing-given-name cases. Returns: - "exact": canonical strings match -
 * "near": surname matches AND (given matches OR one is missing) - "different":
 * no match
 */
