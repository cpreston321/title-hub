...
/// Mirrors the server's verifySignature in convex/http.ts:
/// HMAC-SHA256 over `${timestamp}.${rawBody}`, hex-encoded, sent as
/// `X-Title-Signature: sha256=<hex>`. Server rejects timestamps further
/// than 5 minutes from its clock, so callers should sign close to send.
